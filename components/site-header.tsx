    ...
    <nav className="hidden md:flex gap-1">
      {NAV_ITEMS.map((item) => (
        <Link key={item.href} href={item.href}>
          <Button
            variant="ghost"
            className="text-primary hover:text-primary hover:bg-primary/10 px-3 h-9"
          >
            <span className="text-primary/70 mr-1.5">&gt;</span> {item.label}
          </Button>
        </Link>
      ))}
    </nav>
      ...
      >
        <div className="flex flex-col h-full">
          <div className="border-b border-primary/20 p-4 flex items-center justify-between">
            <BrandLink onClick={() => setIsMenuOpen(false)} />
            <Button
              variant="ghost"
              size="icon"
          ...
          <nav className="flex flex-col gap-0 flex-1">
            {NAV_ITEMS.map((item) => (
              <Link
                ...
                onClick={() => setIsMenuOpen(false)}
              >
                <span className="text-primary/70 mr-2">&gt;</span> {item.label}
              </Link>
            ))}
          </nav>
